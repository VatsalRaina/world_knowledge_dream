...
parser.add_argument('--use_context', type=str, default='yes', help='Whether access is allowed to the context')
...
def main(args):
    ...
    for item in train_data:
        context = " ".join(item["dialogue"])
        question = item["question"]
        lab = item["choice"].index(item["answer"])
        labels.append(lab)
        four_inp_ids = []
        four_tok_type_ids = []
        for i, ans in enumerate(item["choice"]):
            if args.use_context == 'yes':
